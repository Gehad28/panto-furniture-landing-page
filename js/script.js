...
// ____ Third Section ____

const btns = document.querySelectorAll(".btn-sm");

const deactivateBtns = () => {
    btns.forEach(btn => {
        btn.classList.remove("btn-sm-active");
    });
}

const handleFurnitureBtns = (e) => {
    deactivateBtns();
    e.target.classList.add("btn-sm-active");
}

btns.forEach(btn => {
    btn.addEventListener("click", handleFurnitureBtns);
});
